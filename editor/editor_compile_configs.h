#pragma once

#define LUMINA_EDITOR_RESOURCES_PATH "resources/"
#define LUMINA_EDITOR_PROJECT_SCENES_DEFAULT_PATH "scenes"
#define LUMINA_EDITOR_PROJECT_ASSETS_DEFAULT_PATH "assets"
#define LUMINA_EDITOR_PROJECT_SCRIPTS_DEFAULT_PATH "scripts"
#define LUMINA_EDITOR_PROJECT_BUILD_DIR_PATH "temp_build"
#define LUMINA_EDITOR_PROJECT_BUILD_CONTENT_DIR "content"
#define LUMINA_EDITOR_PROJECT_BUILD_BIN_DIR "bin"
#define LUMINA_EDITOR_PROJECT_MONO_BINS_DIR "mono_res"
#define LUMINA_EDITOR_PROJECT_ASSET_PACKAGE_NAME "compiled_resources.package"

namespace lumina_editor_configs
{
	constexpr const char* EDITOR_VERSION = "0.0.1";
	constexpr const char* EDITOR_NAME = "Lumina Editor";
	constexpr const char* EDITOR_WINDOW_NAME = "Lumina Editor";
	constexpr float EDITOR_DEFAULT_WIDTH = 1280;
	constexpr float EDITOR_DEFAULT_HEIGHT = 1024;
}