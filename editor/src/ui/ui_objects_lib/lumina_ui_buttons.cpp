...
{
	bool lumina_ui_buttons::img_txt_button(const std::string& str, const lumina::texture& texture, const float txt_offset, const ImVec2 size)
	{
		...
		ImGui::SetCursorPosX(txt_offset == -1.0f ? ImGui::GetCursorPosX() + (size.x / 2.0f) - (str_pixel_size.x / 2.0f) : ImGui::GetCursorPosX() + txt_offset);
